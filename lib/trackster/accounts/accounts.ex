defmodule Trackster.Accounts do
  @moduledoc """
  ...
  alias Trackster.Accounts.{User, UserToken, UserNotifier, PrivateKey}
  ...
  def create_user(attrs) do
    changeset =
      ...
      |> User.maybe_put_private_key()
      |> IO.inspect(label: "Here's the changeset")

    Multi.new()
    |> Multi.insert(:user, changeset)
    |> Multi.run(:key, fn repo, %{user: user} ->
      case repo.get(PrivateKey, user.private_key_id) do
        nil -> {:error, :not_found}
        key -> {:ok, key}
      end
    end)
    |> Multi.update(:update_key, fn %{key: key} ->
      active_private_key_changeset(key)
    end)
    |> Repo.transaction()
    |> case do
      {:ok, %{user: user}} -> {:ok, user}
      {:error, :user, changeset, _} -> {:error, User.translate_private_key_erros(changeset)}
    end
  end
  ...
  def register_user(attrs) do
    ...
    |> User.registration_changeset(attrs)
    |> Repo.insert()
    |> User.translate_private_key_erros()
  end
  ...
  ## Examples
      ...
      %Ecto.Changeset{data: %PrivateKey{}}

  """
  def change_private_key(%PrivateKey{} = private_key, attrs \\ %{}) do
    PrivateKey.changeset(private_key, attrs)
  end

  def active_private_key_changeset(%PrivateKey{} = private_key) do
    attrs = %{
      "is_active" => true,
      "renewal_at" =>
